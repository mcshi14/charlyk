...
local radar =
{
	info = "~y~Radar iniciado, esperando respuesta... PULSA E PARA HACER UNA FOTO ",
}
		...
		Citizen.Wait(1) 
			if IsControlJustPressed(1, cfg.menuopen) then --246 = Y
     			if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey(cfg.radargun) then
					if shown == true then 
						shown = false 
						radar.info = string.format("~y~Radar iniciado, esperando respuesta... PULSA E PARA HACER UNA FOTO ")
					else 
						shown = true 	
     				end
   				end
				...
				if IsEntityAVehicle(e) then
					if IsPlayerFreeAiming(playerId) then
						...
						if cfg.metric == true then
							...
							radar.info = string.format("~y~Mat: ~w~%s  ~y~Modelo: ~w~%s  ~y~Velocidad: ~w~%s km/h", fplate, fmodel, math.ceil(fvspeed))
						else 

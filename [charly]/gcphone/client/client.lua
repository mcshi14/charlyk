...
AddEventHandler("gcPhone:receiveMessage", function(message, numeroid, comentario, myId)
  -- SendNUIMessage({event = 'updateMessages', messages = messages})
  SendNUIMessage({event = 'newMessage', message = message})
  if message.owner == 0 then
    local text = '~o~Nowa wiadomość'
    if ShowNumberNotification == true then
      text = '~o~Nowa wiadomość do ~y~'.. message.transmitter
	  foto = false
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = '~o~Nowa wiadomość od ~g~'.. contact.display
		  foto = true
          break
        end
      end
	...
	print("valor de myid")
	print(myId)
	local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(myId))
	ESX.ShowAdvancedNotification("~y~Nowa wiadomość :~s~", comentario, text, mugshotStr, 1)
    ...
    SetNotificationMessage("CHAR_CHAT_CALL", "CHAR_CHAT_CALL", false, 1, "~y~Nowa wiadomość :~s~", "message");
...
RegisterNUICallback('sendMessage', function(data, cb)
  ...
  local myId = PlayerId()
  TriggerServerEvent('gcPhone:sendMessage', data.phoneNumber, data.message, myId)
end)

...
function getPhoneRandomNumber()
    return '0' .. math.random(600000,699999)
end
...
function addMessage(source, identifier, phone_number, message, myId)
    local sourcePlayer = tonumber(source)
    local otherIdentifier = getIdentifierByPhoneNumber(phone_number)
    local myPhone = getNumberPhone(identifier)
	local numeroid = source
	local comentario = message
	print("LAS VARIABLES HAN SIDO CAMBIADAS")
	print(numeroid)
	print(comentario)
    if otherIdentifier ~= nil then 
        ...
        getSourceFromIdentifier(otherIdentifier, function (osou)
            if tonumber(osou) ~= nil then 
                ...
                TriggerClientEvent("gcPhone:receiveMessage", tonumber(osou), tomess, numeroid, comentario, myId)
            end
        end) 
    end
    local memess = _internalAddMessage(phone_number, myPhone, message, 1)
    TriggerClientEvent("gcPhone:receiveMessage", sourcePlayer, memess, numeroid, comentario, myId)
end
...
AddEventHandler('gcPhone:sendMessage', function(phoneNumber, message, myId)
    local sourcePlayer = tonumber(source)
    local identifier = getPlayerID(source)
    addMessage(sourcePlayer, identifier, phoneNumber, message, myId)
end)
